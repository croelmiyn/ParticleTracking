...
public class Particle_Tracker implements PlugInFilter{
		...
		for(int t=0; t<nFrames; t++){
			...
			for(int i=0; i<partCoords.size(); i++){
				...
				if(hdlPt.size() > 3.14*threshSize*threshSize){ // is a particle
					...
					//////////// centroid method for each subpoint
					x0 = new double[ptNb+1];
					y0 = new double[ptNb+1];
					norm = new double[ptNb+1];
					
					for(int j=0; j<partList.length; j++){
						x0[partList[j]] += (double) (xList[j]*intList[j]);
						y0[partList[j]] += (double) (yList[j]*intList[j]);
					...
					for(int j=1; j<=ptNb; j++){
						if(norm[j]!=0){
							x0[j] /= norm[j];
							y0[j] /= norm[j];
							PtInT[t].add(new Point(t,x0[j],y0[j],partSize[j]));
							//IJ.log(""+x0[j]+"  "+y0[j]);
						}
					}
				} // is part
		...
		try {
			...
			for (int t=0;t<nFrames; t++){
				...
				for(int i=0; i<PtInT[t].size(); i++){
					...
					buffer = ""+pt.getT()+"\t"+pt.getY()+"\t"+pt.getX()+"\t"+pt.getSize(); // x and y inverted as in Mosaic old version
					buffer += lineSep;
